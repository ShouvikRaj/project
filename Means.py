...
def TempMean():
    ...
    ax.set_title("Mean Temperature Distribution (January 1958 to December 2019)", fontsize=14)
...
def WindMean():
    ...
    # Use pcolormesh with improved quality
    im = ax.pcolormesh(x, y, mean_air_speed, 
                    cmap='rainbow',
                    shading='gouraud')  

    contour_levels = np.linspace(np.min(mean_air_speed), np.max(mean_air_speed), 24)
    cs = ax.contour(x, y, mean_air_speed, levels=contour_levels, 
                    colors='black', alpha=0.5, linewidths=0.5)
    ax.clabel(cs, inline=True, fontsize=10, fmt='%1.0f')  # Increased font size
    ...
    ax.set_title("Mean Eastward Wind Speed Distribution (January 1958 to December 2019)", fontsize=14)
